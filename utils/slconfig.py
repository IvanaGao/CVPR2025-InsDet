...
class SLConfig(object):
    ...
    def merge_from_dict(self, options):
        ...
        Examples:
            >>> options = {'model.visual_backbone.depth': 50,
            ...            'model.visual_backbone.with_cp':True}
            >>> cfg = Config(dict(model=dict(visual_backbone=dict(type='ResNet'))))
            >>> cfg.merge_from_dict(options)
            >>> cfg_dict = super(Config, self).__getattribute__('_cfg_dict')
            >>> assert cfg_dict == dict(
            ...     model=dict(visual_backbone=dict(depth=50, with_cp=True)))
