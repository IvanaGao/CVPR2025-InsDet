...
class NestedTensor(object):
    ...
    def half(self):
        self.tensors = self.tensors.half()
...
def init_distributed_mode(args):
    if 'WORLD_SIZE' in os.environ and os.environ['WORLD_SIZE'] != '': # 'RANK' in os.environ and 
        ...
        # Single node
        #   python -m torch.distributed.launch --nproc_per_node=8 main_train.py --world-size 1 --rank 0 ...
        # Multi nodes
        #   python -m torch.distributed.launch --nproc_per_node=8 main_train.py --world-size 2 --rank 0 --dist-url 'tcp://IP_OF_NODE0:FREEPORT' ...
        #   python -m torch.distributed.launch --nproc_per_node=8 main_train.py --world-size 2 --rank 1 --dist-url 'tcp://IP_OF_NODE0:FREEPORT' ...
...
def clean_words_or_phrase(name):
    ...
    if '/' in name:
        ...
        for item in items:
            if '/' in item:
                name = name.replace(item, random.choice(item.split('/')))

    name = re.sub(r"\(.*\)", "", name)  # 将字符串中所有括号 () 及括号内的内容删除。例如，"example (text)" 变为 "example"
    name = re.sub(r"_", " ", name)      # 将字符串中的所有下划线 _ 替换为空格 " "，例如，"example_text" 变为 "example text"
    name = re.sub(r"-", " ", name)      # 将字符串中的所有连字符 - 替换为空格 " "，例如，"example-text" 变为 "example text"
    name = re.sub(r"  ", " ", name)     # 将字符串中的双空格 " " 替换为单空格 " "，例如，"example  text" 变为 "example text"
    name = name.strip().lower()         # 去除字符串两端的空格，lower() 将整个字符串转换为小写
...
def frozen_parameter(model=None, args=None, logger=None):
    ...
    if logger is None:
        print('args.freeze_keywords = {}'.format(args.freeze_keywords))
    else:
        logger.info('args.freeze_keywords = {}'.format(args.freeze_keywords))
...
if __name__ == '__main__':
    ...
    temp = clean_words_or_phrase('abc tortoise/turtle efr')
    print()
