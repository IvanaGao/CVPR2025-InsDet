...
class OVDINO(nn.Module):
    ...
    def __init__(
        ...
        lang_embed_dim = self.language_backbone.lang_model.config.hidden_size   # 兼容不同尺寸bert
        # self.lang_embed_proj = nn.Linear(lang_embed_dim, embed_dim, bias=True)
        self.class_embed = ClassEmbed(lang_embed_dim, embed_dim)
        # self.class_embed.lang_embed_proj = nn.Identity()
    ...
    def forward(self, batched_inputs):
        ...
        if not self.training:
            ...
            if isinstance(names[0], list):
                ...
                import sys
                sys.exit(1)
                # text_embed = self.language_backbone
        else:
        ...
        # 维度对齐
        # text_embed = self.lang_embed_proj(text_embed)
