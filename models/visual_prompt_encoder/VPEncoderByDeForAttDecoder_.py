...
class VPEncoderByDeForAttDecoder(nn.Module):
    ...
    def __init__(self, layer_num=3, embed_dim=256, feature_levels_num=4, batch_first=True, device='cpu'):
        super().__init__()
        self.device = device
    ...
    def get_vp_reference_points(reference_points, spatial_shapes, valid_ratios, device):
        ...
        ref_x = reference_points[:, :, 0] / (valid_ratios[:, None, 0, 0] * W)
        ref_y = reference_points[:, :, 1] / (valid_ratios[:, None, 0, 1] * H)
    ...
    def forward(self, multi_level_feats, multi_level_masks, multi_level_pos_embeds, reference_points, **kwargs):
