...
# except:
#     from sd_minimal import selective_scan_chunk_fn

from models.visual_backbone.vmamba.csm_triton import cross_scan_fn, cross_merge_fn
from models.visual_backbone.vmamba.csms6s import selective_scan_fn, selective_scan_flop_jit
from models.visual_backbone.vmamba.ssd_minimal import selective_scan_chunk_fn
print()
...
class Backbone_VSSM(VSSM):
    ...
    def forward(self, x):
        ...
        # 兼容swin输出
        outs = {'p{}'.format(map_id): outs[i] for i, map_id in enumerate(self.out_indices)}
...
def vmamba_tiny_s2l5(channel_first=True):
    return VSSM(
        depths=[2, 2, 5, 2], dims=96, drop_path_rate=0.2, ssm_ratio=2.0,
...
def vmamba_small_s2l15(channel_first=True):
    return VSSM(
        depths=[2, 2, 15, 2], dims=96, drop_path_rate=0.3, ssm_ratio=2.0,
        ssm_d_state=1, ssm_dt_rank="auto", ssm_act_layer="silu",
        patch_size=4, in_chans=3, num_classes=1000,
        ssm_conv=3, ssm_conv_bias=False, ssm_drop_rate=0.0,
        ssm_init="v0", forward_type="v05_noz",
...
def vmamba_base_s2l15(channel_first=True):
    return VSSM(
        depths=[2, 2, 15, 2], dims=128, drop_path_rate=0.6, ssm_ratio=2.0,
        ssm_d_state=1, ssm_dt_rank="auto", ssm_act_layer="silu",
        patch_size=4, in_chans=3, num_classes=1000,
        ssm_conv=3, ssm_conv_bias=False, ssm_drop_rate=0.0,
        ssm_init="v0", forward_type="v05_noz",
...
def vmamba_tiny_s1l8(channel_first=True):
    return VSSM(
        depths=[2, 2, 8, 2], dims=96, drop_path_rate=0.2, ssm_ratio=1.0,
        ssm_d_state=1, ssm_dt_rank="auto", ssm_act_layer="silu",
        patch_size=4, in_chans=3, num_classes=1000,
        ssm_conv=3, ssm_conv_bias=False, ssm_drop_rate=0.0,
        ssm_init="v0", forward_type="v05_noz",
...
def vmamba_tiny_s1l8_backbone(channel_first=True):
...
def vmamba_small_s1l20(channel_first=True):
    return VSSM(
        depths=[2, 2, 20, 2], dims=96, drop_path_rate=0.3, ssm_ratio=1.0,
        ssm_d_state=1, ssm_dt_rank="auto", ssm_act_layer="silu",
        patch_size=4, in_chans=3, num_classes=1000,
        ssm_conv=3, ssm_conv_bias=False, ssm_drop_rate=0.0,
        ssm_init="v0", forward_type="v05_noz",
...
def vmamba_base_s1l20(channel_first=True):
    return VSSM(
        depths=[2, 2, 20, 2], dims=128, drop_path_rate=0.5, ssm_ratio=1.0,
        ssm_d_state=1, ssm_dt_rank="auto", ssm_act_layer="silu",
        patch_size=4, in_chans=3, num_classes=1000,
        ssm_conv=3, ssm_conv_bias=False, ssm_drop_rate=0.0,
        ssm_init="v0", forward_type="v05_noz",
...
if __name__ == "__main__":
    ...
    model_ref = vmamba_tiny_s1l8_backbone()
    ...
    temp1 = model_ref(inp)
    inp = torch.randn((16, 3, 368, 368)).cuda()
    temp2 = model_ref(inp)
    print()
