...
class ChannelMapper(nn.Module):
    """Channel Mapper for reduce/increase channels of visual_backbone features. Modified
    from `mmdet <https://github.com/open-mmlab/mmdetection/blob/master/mmdet/models/necks/channel_mapper.py>`_.

    This is used to reduce/increase the channels of visual_backbone features.

    Args:
        input_shape (Dict[str, ShapeSpec]): A dict which contains the visual_backbone features meta infomation,
            e.g. ``input_shape = {"res5": ShapeSpec(channels=2048)}``.
        in_features (List[str]): A list contains the keys which maps the features output from the visual_backbone,
            e.g. ``in_features = ["res"]``.
    ...
    def forward(self, inputs):
        ...
        Args:
            inputs (Dict[str, torch.Tensor]): The visual_backbone feature maps.
