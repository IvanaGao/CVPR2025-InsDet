...
def log_timm_feature_info(feature_info):
    """Print feature_info of timm visual_backbone to help development and debug.
    Args:
        feature_info (list[dict] | timm.models.features.FeatureInfo | None):
            feature_info of timm visual_backbone.
    """
    logger = setup_logger(name="timm visual_backbone")
    if feature_info is None:
        logger.warning("This visual_backbone does not have feature_info")
    elif isinstance(feature_info, list):
        for feat_idx, each_info in enumerate(feature_info):
            logger.info(f"visual_backbone feature_info[{feat_idx}]: {each_info}")
    else:
        try:
            logger.info(f"visual_backbone out_indices: {feature_info.out_indices}")
            logger.info(f"visual_backbone out_channels: {feature_info.channels()}")
            logger.info(f"visual_backbone out_strides: {feature_info.reduction()}")
        except AttributeError:
            logger.warning("Unexpected format of visual_backbone feature_info")


class TimmBackbone(Backbone):
    """A wrapper for using visual_backbone from timm library.
